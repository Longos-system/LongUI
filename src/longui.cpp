...
void LongUI::initializeEngine(QQmlEngine *engine, const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("LongUI"));

    int fontId = QFontDatabase::addApplicationFont(":/longui/kit/images/fonticons/FluentSystemIcons-Regular.ttf");
...
void LongUI::registerTypes(const char *uri)
{
    Q_ASSERT(QLatin1String(uri) == QLatin1String("LongUI"));

    qmlRegisterSingletonType<ThemeManager>("LongUI.Core", 1, 0, "ThemeManager", [](QQmlEngine *engine, QJSEngine *scriptEngine) -> QObject * {
        Q_UNUSED(engine)
...
QUrl LongUI::componentUrl(const QString &fileName) const
{
    return QUrl(QStringLiteral("qrc:/longui/kit/") + fileName);
}
